...
public class TransactionsServiceImpl implements TransactionsService {
    ...
    public Transaction initiateTransaction(Transaction transaction) {
        ...
        AccountDto fromAccount = restTemplate.getForObject("http://localhost:8081/accounts/" + transaction.getFrom_account_id(), AccountDto.class);
        AccountDto toAccount = restTemplate.getForObject("http://localhost:8081/accounts/" + transaction.getTo_account_id(), AccountDto.class);
    ...
    public Transaction executeTransaction(Transaction transaction) {
        ...
        restTemplate.postForObject("http://localhost:8081/accounts/" + transaction.getFrom_account_id() + "/withdraw", transaction.getAmount(), Void.class);
        restTemplate.postForObject("http://localhost:8081/accounts/" + transaction.getTo_account_id() + "/deposit", transaction.getAmount(), Void.class);
        UUID transactionId = transaction.getId();
        transactionSuccess(transactionId);
        return transaction;
    }
